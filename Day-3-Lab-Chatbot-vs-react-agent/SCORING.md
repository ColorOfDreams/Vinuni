...
## ExamForge AI Submission Checklist

Use this section to map the implemented project to the rubric.

| Rubric Item | Evidence in Project |
| :--- | :--- |
| Chatbot Baseline | `src/chatbot.py`, `runLLMWorkflow()` in `src/frontend/lib/exam-workflows.ts` |
| Agent v1 Working | `src/agent/agent.py`, `runAgentWorkflow()` |
| Agent v2 Improved | Blueprint validation, mistake advice, final summary, frontend logs |
| Tool Design Evolution | `src/tools/education_tools.py`, `src/frontend/lib/exam-workflows.ts` |
| Trace Quality | Agent trace UI and `logs/frontend-comparison.log` |
| Evaluation & Analysis | `EVALUATION.md`, `report/FRONTEND_WORKFLOW_REPORT.md` |
| Flowchart & Insight | `report/group_report/TEMPLATE_GROUP_REPORT.md` |
| Code Quality | Modular frontend files, typed workflow functions, `npm run build` pass |

Bonus evidence:

- **Extra Monitoring**: frontend comparison log records latency, mode, trace steps, score and percentage.
- **Failure Handling**: Agent validates exam structure and preserves foundation questions even for hard exams.
- **Live System Demo**: run `cd src/frontend && npm run dev -- --port 3001`.
