...
- If the user asks to draft, write, prepare, or compose content for later publishing, do not treat it as a send/publish action.
- Vietnamese draft phrases such as "soạn", "viết nháp", "chuẩn bị", or "để lát nữa đăng" mean draft intent, not immediate publishing.
- For draft intent, do not call `send` and do not ask for yes/no send confirmation. Produce the draft normally, using research tools only if current information is needed.     
